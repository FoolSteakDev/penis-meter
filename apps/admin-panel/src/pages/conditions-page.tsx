...
export default function ConditionsPage() {
  ...
  async function handleChangeDeltaMode(
    ...
    try {
      const updated = await api.updateCondition(condition.id, {
        deltaMode,
        fixedValues:
          deltaMode === "fixed_list" && condition.fixedValues.length === 0
            ? [0]
            : undefined,
      });
    ...
    <div className="space-y-8">
        ...
        <table className="w-full text-left text-sm">
          <thead>
            <tr className="bg-navy text-cream">
              ...
              <th className="w-14 px-3 py-3 text-center font-semibold">
                Видалити
              </th>
            </tr>
          </thead>
                  ...
                  <div className="flex flex-col gap-1.5">
                    <div className="inline-flex w-fit overflow-hidden rounded-md border border-navy/20 text-xs">
                      <button
                        type="button"
                        onClick={() =>
                          handleChangeDeltaMode(condition, "range")
                        }
                      ...
                      <button
                        type="button"
                        onClick={() =>
                          handleChangeDeltaMode(condition, "fixed_list")
                        }
                    ...
                    {condition.deltaMode === "fixed_list" ? (
                      <EditableFixedValuesField
                        ...
                        onSave={async (fixedValues) => {
                          const updated = await api.updateCondition(
                            condition.id,
                            { fixedValues },
                          );
                          patchLocal(condition.id, updated);
      ...
      <div className="rounded-xl border border-gold/40 bg-white p-6 shadow-sm">
        ...
        <p className="mb-4 text-xs text-navy/60">
          ...
          {creatableCodes.length === 0 && availableCodes.length > 0
            ? " (усі вже використані нижче)"
            : ""}
          . Будь-який інший код (напр.{" "}
          <span className="font-mono">jackpot</span>) створює звичайну умову без
          спецлогіки: дельта або рандом у діапазоні мін..макс, або випадкове
          число зі списку фіксованих значень.
        </p>
